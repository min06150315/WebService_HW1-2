...
public class ChampionCRUD implements ICRUD<Champion> {
    // Champion 객체를 저장하는 리스트. 참조 변경 방지를 위해 final로 선언
    ...
    public int create(Champion champion) {
        // 리스트 크기를 이용해 새로운 Champion의 ID 설정
    ...
    public List<Champion> list() {
        // Collections.unmodifiableList를 사용하여 읽기 전용 리스트 반환
    ...
    public int update(Champion champion) {
        ...
        return 0; // ID가 일치하는 Champion이 없을 경우 0 반환
    }
    ...
    public int delete(int id) {
        ...
        // 리스트를 순회하여 삭제할 Champion의 인덱스를 찾기
        for (int i = 0; i < championList.size(); i++) {
        ...
        return 0; // 해당 ID의 Champion이 없을 경우 0 반환
    }
}
